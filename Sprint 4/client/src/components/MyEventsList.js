...
export default function MyEventsList({ events = [], onRefresh, onExport }) {
                ...
                <table className="w-full text-sm border-collapse">
                    ...
                    {events.map((ev) => (
                        ...
                        >
                            ...
                            <td className="p-2">
                                {fmt(ev.startTime || ev.start)} – {fmt(ev.endTime || ev.end)}
                            </td>
                            <td className="p-2">
            ...
            {selected && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 relative animate-fadeIn">
                        ...
                        <h2 className="text-xl font-semibold mb-3">{selected.title}</h2>

                        <div className="text-sm text-gray-700 space-y-2">
                            <p>
                                <b>📅 Schedule:</b> {fmt(selected.startTime || selected.start)} –{" "}
                                {fmt(selected.endTime || selected.end)}
                            </p>
                            <p>
                                <b>📍 Location:</b> {selected.location || "Not specified"}
                            </p>
